...
| Customer services and consumables | Every sold service remains pending until a Cashier or Admin completes it. Completion records the consumables used, or requires explicit confirmation that none were used, in the same transaction. Unfinished services block manual shift closing. The combined POS workspace also manages `ml`/`gm` package configuration, transfers, balances, valued ledger history, usage, and completion reports. Completed services are final in the operational UI. |
