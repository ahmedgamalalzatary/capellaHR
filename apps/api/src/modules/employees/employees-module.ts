...
import { createEmployeeService, type EmployeeFinancialLifecycle, type EmployeeOpenWorkCapability } from './employees-service.js';
import { createErpEmployeeCapability } from './erp-employee-capability.js';
export const createEmployeesModule = (database: ReturnType<typeof createDatabase>, maxImageBytes: number, attendance?: { hasOpenSession(id: number, context?: unknown): Promise<boolean>; hasAnyOpenSession(id: number, context?: unknown): Promise<boolean> }, existingRepository?: ReturnType<typeof createDrizzleEmployeeRepository>, deviceLifecycle?: { revokeEmployee(id: number, context?: unknown): Promise<void> }, financialLifecycle?: EmployeeFinancialLifecycle, existingUploadStore?: EmployeeUploadStore, openWork?: EmployeeOpenWorkCapability) => {
  ...
  return {
    repository,
    uploadStore,
    service: createEmployeeService(repository, attendance, deviceLifecycle, financialLifecycle, openWork),
    erp: createErpEmployeeCapability(repository),
  };
};
