 ...
 * first page. Only for small admin datasets (employees of one company).
 *
 * `identity` names the field that tells two rows apart, and it defaults to `id`.
 * It exists because not every list is keyed the same way: consumable balances,
 * for instance, carry `productId` and no `id` of their own.
 *
