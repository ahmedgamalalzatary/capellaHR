...
const expense = { id: 10, branchId: 2, name: 'كهرباء', amount: '125.50', expenseDate: '2026-08-05', description: 'مستلزمات', actingAccountId: 7, actingUsername: 'admin', kind: 'expense', status: 'active', reversalOfId: null, supersedesId: null, correctionReason: null, createdAt: '2026-08-05T10:00:00Z' };
...
describe('ExpensesView', () => {
  it('finds an expense by its name or notes in the history filter', async () => {
    ...
    fireEvent.change(screen.getByLabelText('بحث بالاسم أو الوصف'), { target: { value: '  كهرباء  ' } });

    await waitFor(() => expect(mocks.list)
      .toHaveBeenCalledWith(expect.objectContaining({ search: 'كهرباء' })));
  });
  ...
  it('creates and filters branch-scoped expenses', async () => {
    ...
    await waitFor(() => expect(mocks.create).toHaveBeenCalledWith(expect.objectContaining({ branchId: 2, name: 'كهرباء', amount: '125.50' })));
  ...
  it('allows an active correction replacement to be corrected again', async () => {
    ...
    await screen.findByText('كهرباء'); expect(screen.getByRole('button', { name: 'تصحيح' })).toBeTruthy();
  });
  ...
  it('records an expense for a cashier without asking which branch', async () => {
    ...
    await waitFor(() => expect(mocks.create).toHaveBeenCalledWith({ branchId: undefined, name: 'كهرباء', amount: '125.50', expenseDate: '2026-08-05', description: 'مستلزمات' }));
