...
export function SuppliersPurchasesView() {
  ...
  const cancel = useMutation({
    ...
    onSuccess: async () => { setCancelling(null); setReason(''); await refreshPurchase(); },
  });
      ...
      <Card><CardContent className="space-y-3 pt-5">
        ...
        {suppliers.isError || activeProducts.isError ? <EmptyState title="تعذر تحميل خيارات المشتريات" action={<Button onClick={() => { void suppliers.refetch(); void activeProducts.refetch(); }}>إعادة المحاولة</Button>} /> : suppliers.isPending || activeProducts.isPending ? <p className="text-sm text-muted">جارٍ تحميل خيارات المشتريات…</p> : <>
          <div className="grid gap-2 md:grid-cols-2">
            <Label>المورد للمشتريات<select className="mt-1 h-10 w-full rounded-control border border-line bg-paper px-3" value={supplierId} onChange={(event) => { setIdempotencyKey(crypto.randomUUID()); setSupplierId(event.target.value); }}><option value="">اختر المورد</option>{activeSuppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></Label>
            <Label>تاريخ المشتريات<Input type="date" value={purchaseDate} onChange={(event) => { setIdempotencyKey(crypto.randomUUID()); setPurchaseDate(event.target.value); }} /></Label>
          </div>
          {lines.map((line, index) => <div key={line.key} className="grid gap-2 md:grid-cols-4">
            ...
            <Button variant="ghost" disabled={lines.length === 1} onClick={() => { setIdempotencyKey(crypto.randomUUID()); setLines((current) => current.filter((entry) => entry.key !== line.key)); }}>حذف البند {index + 1}</Button>
          </div>)}
          <Button variant="ghost" onClick={() => { setIdempotencyKey(crypto.randomUUID()); setLines((current) => [...current, blankLine(lineKey)]); setLineKey((value) => value + 1); }}>إضافة بند</Button>
