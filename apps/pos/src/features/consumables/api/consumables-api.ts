...
export interface ConsumableServiceExecution { id: number; status: 'pending' | 'completed' | 'overdue' | 'canceled'; queueNumber: number; cashierSessionId: number; invoiceId: number; invoiceNumber: string; clientName: string | null; clientPhone: string | null; serviceId: number; serviceName: string; employeeId: number | null; employeeName: string | null; createdAt: string; completedAt: string | null }
