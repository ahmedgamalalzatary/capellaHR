...
describe('devices HTTP API', () => {
  it('fails closed instead of accepting unverified registration material', async () => {
    ...
    const response = await request(app(null, deviceService)).post(`/api/v1/devices/pairings/${'x'.repeat(32)}/complete`).send({ credentialId: 'c', publicKey: 'p', installationMarker: 'm', browser: 'Chrome', platform: 'Android' });
