...
CREATE INDEX `auth_sessions_account_active_idx` ON `auth_sessions` (`account_id`,`revoked_at`);--> statement-breakpoint
ALTER TABLE `auth_sessions` ADD CONSTRAINT `auth_sessions_account_id_accounts_id_fk` FOREIGN KEY (`account_id`) REFERENCES `accounts`(`id`) ON DELETE no action ON UPDATE no action;
