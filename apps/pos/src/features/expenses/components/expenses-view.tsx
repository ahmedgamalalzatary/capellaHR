...
export function ExpensesView() {
  ...
  const activeCategories = useQuery({
    queryKey: ['expense-categories', branchId, 'active'],
  ...
  const refresh = () => client.invalidateQueries({ queryKey: expenseQueryKeys.all });
  const create = useMutation({ mutationFn: () => createExpense({ branchId, categoryId: categoryId!, amount, expenseDate, description }), onSuccess: async () => { setAmount(''); setDescription(''); await refresh(); } });
  const correction = useMutation({ mutationFn: () => correctExpense(correcting!.id, { branchId, categoryId: categoryId!, amount, expenseDate, description, reason }), onSuccess: async () => { setCorrecting(null); setReason(''); setAmount(''); setDescription(''); await refresh(); } });
  ...
  return <div className="mx-auto max-w-7xl space-y-4" dir="rtl">
    ...
    {branchId === undefined ? <EmptyState title="اختر فرعًا لإدارة مصروفاته" /> : <>
      <Card><CardContent className="grid gap-3 pt-5 md:grid-cols-2 xl:grid-cols-5">
        <Label>التصنيف<select aria-label="التصنيف" className="mt-1 h-10 w-full rounded-control border border-line bg-paper px-3" value={categoryId ?? ''} onChange={(event) => setCategoryId(event.target.value ? Number(event.target.value) : undefined)} disabled={activeCategories.isPending || activeCategories.isError}><option value="">اختر التصنيف</option>{activeCategories.data?.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></Label>
        <Label>المبلغ<Input aria-label={correcting ? 'المبلغ الصحيح' : 'المبلغ'} inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /></Label>
        <Label>تاريخ المصروف<Input aria-label="تاريخ المصروف" type="date" value={expenseDate} onChange={(event) => setExpenseDate(event.target.value)} /></Label>
        <Label>الوصف<Input aria-label="الوصف" value={description} onChange={(event) => setDescription(event.target.value)} /></Label>
        {correcting ? <Label>سبب التصحيح<Input aria-label="سبب التصحيح" value={reason} onChange={(event) => setReason(event.target.value)} /></Label> : <div className="self-end"><Button disabled={!canSubmit || create.isPending} onClick={() => create.mutate()}>تسجيل المصروف</Button></div>}
        {correcting ? <div className="flex gap-2 md:col-span-2 xl:col-span-5"><Button disabled={!canSubmit || !reason.trim() || correction.isPending} onClick={() => correction.mutate()}>تأكيد التصحيح</Button><Button variant="ghost" onClick={clearDraft}>إلغاء</Button></div> : null}
