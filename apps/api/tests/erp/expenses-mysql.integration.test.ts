...
beforeAll(async () => {
  const existing = (await database.select({ id: accounts.id }).from(accounts).where(eq(accounts.role, 'admin')).limit(1))[0];
  const accountId = existing?.id ?? Number((await database.insert(accounts).values({ username: `expense-admin-${process.pid}`, passwordHash: 'test-only', role: 'admin', createdAt: new Date(), updatedAt: new Date() }))[0].insertId);
...
afterAll(async () => {
  ...
  try {
    const expenseIds = await database.select({ id: erpExpenses.id }).from(erpExpenses).orderBy(desc(erpExpenses.id));
    for (const expense of expenseIds) await database.delete(erpExpenses).where(eq(erpExpenses.id, expense.id));
  } finally {
    await database.execute(sql.raw("CREATE TRIGGER erp_expenses_reject_delete BEFORE DELETE ON erp_expenses FOR EACH ROW SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'ERP expense facts cannot be deleted'"));
  }
  await database.delete(auditEvents).where(eq(auditEvents.module, 'erp-expenses'));
  if (seededBranchIds.length > 0) {
    await database.delete(erpCategories).where(inArray(erpCategories.branchId, seededBranchIds));
    await database.delete(branches).where(inArray(branches.id, seededBranchIds));
  }
});
