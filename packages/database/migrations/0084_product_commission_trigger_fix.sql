...
BEGIN
  ...
  ELSEIF NEW.entry_type = 'reversal' THEN
    ...
    SELECT original.base_amount, original.amount INTO target_base, target_amount FROM `erp_commission_ledger_entries` original
      WHERE original.id = NEW.reverses_entry_id AND original.entry_type IN ('earned','reassignment_in') AND original.invoice_id = NEW.invoice_id AND original.invoice_line_id = NEW.invoice_line_id;
