...
export function ConsumablesView() {
  ...
  return <section className="space-y-6"><PageHeader title="المستهلكات والخدمات" description="إدارة رصيد ml/gm وإنهاء تقرير مستقل لكل خدمة مباعة." />
    ...
    {!ready ? <EmptyState title="اختر فرعاً للمتابعة" /> : <><Card><CardContent className="p-4"><SectionHeading title="أرصدة المستهلكات" />{balances.isPending ? <LoadingState label="جارٍ التحميل…" /> : <DataTable><THead><TH>المنتج</TH><TH numeric>مخزون البيع</TH><TH numeric>رصيد المستهلك</TH><TH numeric>حجم العبوة</TH></THead><tbody>{balances.data?.items.map((item) => <TR key={item.productId}><TD>{item.productName}</TD><TD numeric>{item.sellableQuantity}</TD><TD numeric>{item.consumableQuantity} {item.unit}</TD><TD numeric>{item.packageSize} {item.unit}</TD></TR>)}</tbody></DataTable>}</CardContent></Card>
      <Card><CardContent className="space-y-4 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><SectionHeading title="تقارير الخدمات" /><Select aria-label="حالة الخدمة" value={status} onChange={(event) => { setStatus(event.target.value as typeof status); setSelected([]); }}><option value="pending">غير منتهية</option><option value="completed">منتهية</option><option value="overdue">متأخرة</option></Select></div>{services.isPending ? <LoadingState label="جارٍ تحميل الخدمات…" /> : <DataTable><THead>{status !== 'completed' ? <TH>اختيار</TH> : null}<TH>الخدمة</TH><TH>الدور</TH><TH>الفاتورة</TH><TH>العميل</TH><TH>الموظف</TH>{status === 'completed' ? <TH>الإجراء</TH> : null}</THead><tbody>{services.data?.items.map((item) => <TR key={item.id}>{status !== 'completed' ? <TD><input aria-label={`اختيار الخدمة ${item.queueNumber}`} type="checkbox" checked={selected.includes(item.id)} onChange={() => setSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : [...ids, item.id])} /></TD> : null}<TD>{item.serviceName}</TD><TD><Badge>{item.queueNumber}</Badge></TD><TD>{item.invoiceNumber}</TD><TD>{item.clientName ?? item.clientPhone ?? '—'}</TD><TD>{item.employeeName ?? '—'}</TD>{status === 'completed' ? <TD><Button size="sm" variant="ghost" onClick={() => setCorrectingId(item.id)}>تصحيح</Button></TD> : null}</TR>)}</tbody></DataTable>}
        {(status !== 'completed' || correctingId) ? <div className="space-y-3 border-t border-line pt-4"><SectionHeading title={correctingId ? 'تصحيح الاستهلاك' : `إنهاء ${selected.length} خدمة`} />{usages.map((usage, index) => <div className="flex flex-wrap gap-2" key={index}><Select aria-label={`المستهلك ${index + 1}`} value={usage.productId} onChange={(event) => setUsages((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, productId: event.target.value ? Number(event.target.value) : '' } : row))}><option value="">بدون مستهلك / اختر منتجاً</option>{balances.data?.items.map((item) => <option key={item.productId} value={item.productId}>{item.productName} ({item.unit})</option>)}</Select><Input aria-label={index === 0 ? 'كمية المستهلك' : `كمية المستهلك ${index + 1}`} type="number" min="0.001" step="0.001" value={usage.quantity} onChange={(event) => setUsages((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, quantity: event.target.value } : row))}/>{usages.length > 1 ? <Button variant="ghost" onClick={() => setUsages((rows) => rows.filter((_, rowIndex) => rowIndex !== index))}>حذف</Button> : null}</div>)}<Button variant="secondary" onClick={() => setUsages((rows) => [...rows, { productId: '', quantity: '' }])}>إضافة مستهلك</Button>{correctingId ? <><Input aria-label="سبب التصحيح" value={correctionReason} onChange={(event) => setCorrectionReason(event.target.value)} /><Button disabled={!correctionReason.trim() || pending} onClick={() => correct.mutate()}>حفظ التصحيح</Button></> : <Button disabled={!selected.length || pending} onClick={() => complete.mutate()}>إنهاء الخدمات المحددة</Button>}</div> : null}{failed ? <FieldError>{errorText(failed.error)}</FieldError> : null}</CardContent></Card></>}
  </section>;
}
