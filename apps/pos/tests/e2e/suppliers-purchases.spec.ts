...
test('Admin manages a supplier, posts exact purchase stock facts, and cancels once', async ({ page }) => {
  let supplier: Record<string, unknown> | undefined; let purchase: Record<string, unknown> | undefined; let postPayload: Record<string, unknown> | undefined; let cancelPayload: Record<string, unknown> | undefined; let productQuantity = 5; const supplierUpdates: Array<Record<string, unknown>> = [];
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/erp/products') return json(route, [{ id: 4, branchId: 2, name: 'شامبو', description: null, sellingPrice: '20.00', lastPurchaseCost: '12.50', lowStockThreshold: 2, quantity: productQuantity, isActive: true, createdAt: '2026-08-01T00:00:00Z', updatedAt: '2026-08-05T10:00:00Z' }], { page: 1, pageSize: 100, total: 1, totalPages: 1 });
    if (path === '/erp/products/movements') return json(route, [], { page: 1, pageSize: 20, total: 0, totalPages: 0 });
  ...
  await page.getByLabel('المورد للمشتريات').selectOption('3'); await page.getByRole('combobox', { name: 'المنتج', exact: true }).click(); await page.getByRole('option', { name: 'شامبو' }).click(); await page.getByLabel('الكمية', { exact: true }).fill('2'); await page.getByLabel('تكلفة الوحدة', { exact: true }).fill('12.50'); await expect(page.getByText('الإجمالي: 25.00 ج.م')).toBeVisible(); await page.getByRole('button', { name: 'ترحيل المشتريات' }).click();
  ...
  await page.getByRole('link', { name: 'الموردون والمشتريات' }).click(); await expect(page).toHaveURL(/\/suppliers$/); await page.getByLabel('الفرع').selectOption('2');
