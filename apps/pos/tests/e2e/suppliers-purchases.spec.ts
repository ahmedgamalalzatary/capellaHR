...
test('Admin manages a supplier, posts exact purchase stock facts, and cancels once', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'المنتجات والمخزون' }).click(); await page.getByLabel('الفرع').selectOption('2'); const productRow = page.getByRole('row').filter({ hasText: 'شامبو' }); await expect(productRow.getByRole('cell', { name: '7', exact: true })).toBeVisible();
  await page.getByRole('link', { name: 'الموردون والمشتريات' }).click(); await page.getByLabel('الفرع').selectOption('2');
