...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE reversal_line.reversal_id = NEW.id AND original_line.item_type = 'service'
      AND (SELECT COALESCE(SUM(ledger.base_amount), 0.00)
        FROM `erp_commission_ledger_entries` ledger
        WHERE ledger.invoice_reversal_id = NEW.id
          AND ledger.invoice_line_id = reversal_line.invoice_line_id
          AND ledger.entry_type = 'reversal')
        <> original_line.unit_price * reversal_line.quantity
  ) OR EXISTS (
    SELECT ledger.id
    FROM `erp_commission_ledger_entries` ledger
    LEFT JOIN `erp_commission_ledger_entries` earned
      ON earned.id = ledger.reverses_entry_id
        AND earned.entry_type IN ('earned', 'reassignment_in')
    LEFT JOIN `erp_invoice_reversal_lines` reversal_line
      ON reversal_line.reversal_id = ledger.invoice_reversal_id
        AND reversal_line.invoice_line_id = ledger.invoice_line_id
    LEFT JOIN `erp_invoice_lines` original_line ON original_line.id = ledger.invoice_line_id
    LEFT JOIN `erp_service_queue_entries` ticket ON ticket.id = ledger.service_queue_entry_id
    WHERE ledger.invoice_reversal_id = NEW.id AND ledger.entry_type = 'reversal'
      AND (earned.id IS NULL OR reversal_line.id IS NULL OR original_line.id IS NULL
        ...
        OR reversal_line.invoice_id <> NEW.invoice_id
        OR reversal_line.branch_id <> NEW.branch_id
        OR original_line.invoice_id <> NEW.invoice_id
        OR original_line.branch_id <> NEW.branch_id
        OR (ledger.service_queue_entry_id IS NULL AND (
          ledger.base_amount <> original_line.unit_price * reversal_line.quantity
          OR -ledger.amount <> ROUND(
            (COALESCE((SELECT SUM(prior.base_amount)
              ...
              WHERE prior.reverses_entry_id = ledger.reverses_entry_id
                AND prior.entry_type = 'reversal'
                AND prior_reversal.status = 'finalized'), 0.00) + ledger.base_amount)
            ...
            COALESCE((SELECT SUM(prior.base_amount)
              ...
              WHERE prior.reverses_entry_id = ledger.reverses_entry_id
                AND prior.entry_type = 'reversal'
                AND prior_reversal.status = 'finalized'), 0.00)
              * earned.commission_rate_snapshot / 100, 2
          )
        ))
        OR (ledger.service_queue_entry_id IS NOT NULL AND (
          ticket.id IS NULL OR ticket.invoice_line_id <> ledger.invoice_line_id
          OR ticket.invoice_id <> NEW.invoice_id
          OR ledger.base_amount <> original_line.unit_price
          OR -ledger.amount <> ROUND(
            original_line.unit_price * (SELECT COUNT(*)
              FROM `erp_service_queue_entries` earlier
              WHERE earlier.invoice_line_id = ticket.invoice_line_id
                AND earlier.queue_number <= ticket.queue_number)
              * earned.commission_rate_snapshot / 100, 2
          ) - ROUND(
            original_line.unit_price * ((SELECT COUNT(*)
              FROM `erp_service_queue_entries` earlier
              WHERE earlier.invoice_line_id = ticket.invoice_line_id
                AND earlier.queue_number <= ticket.queue_number) - 1)
              * earned.commission_rate_snapshot / 100, 2
          )
        ))
      )
  ) OR EXISTS (
    SELECT ledger.service_queue_entry_id
    FROM `erp_commission_ledger_entries` ledger
    JOIN `erp_invoice_reversals` reversal ON reversal.id = ledger.invoice_reversal_id
    WHERE ledger.invoice_id = NEW.invoice_id AND ledger.entry_type = 'reversal'
      AND ledger.service_queue_entry_id IS NOT NULL
      AND (reversal.status = 'finalized' OR reversal.id = NEW.id)
    GROUP BY ledger.service_queue_entry_id
    HAVING COUNT(*) > 1
  ) THEN
