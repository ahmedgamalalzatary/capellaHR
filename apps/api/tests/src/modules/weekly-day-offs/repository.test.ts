...
import { createDrizzleWeeklyDayOffRepository } from "../../../../src/modules/weekly-day-offs/repository";
...
describe("weekly day off repository", () => {
  it("creates and lists weekly day off assignments", async () => {
    ...
    const rows = await repository.listAssignments(1, "2026-06-27");
    expect(rows).toHaveLength(1);
  });

  it("updates assignments and detects attendance conflicts", async () => {
    const repository = createDrizzleWeeklyDayOffRepository({
      db: databaseClient.db
    });
    const created = await repository.createAssignment({
    ...
    const updated = await repository.updateAssignment(created.id, {
      weekStartDate: "2026-06-27",
      dayOffDate: "2026-06-30"
    });

    expect(updated?.dayOffDate).toBe("2026-06-30");
