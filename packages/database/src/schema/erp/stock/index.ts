...
export const stockMovementReasons = [
  'opening_stock', 'count_correction', 'wastage', 'damage', 'sale', 'purchase', 'purchase_cancellation', 'refund', 'void', 'transfer_in',
] as const;
export const stockMovementSourceTypes = ['adjustment', 'sale', 'purchase', 'purchase_cancellation', 'refund', 'void', 'transfer_in'] as const;
...
export const erpStockMovements = mysqlTable('erp_stock_movements', {
  ...
  check(
    ...
    sql`(${table.reason} in ('opening_stock', 'count_correction', 'wastage', 'damage') and ${table.sourceType} = 'adjustment') or (${table.reason} in ('sale', 'purchase', 'purchase_cancellation', 'refund', 'void', 'transfer_in') and ${table.reason} = ${table.sourceType})`,
  ),
  check(
    'erp_stock_movements_direction_consistent',
    sql`${table.reason} = 'count_correction' or (${table.reason} in ('wastage', 'damage', 'sale', 'purchase_cancellation') and ${table.quantityDelta} < 0) or (${table.reason} in ('opening_stock', 'purchase', 'refund', 'void', 'transfer_in') and ${table.quantityDelta} > 0)`,
  ),
]);
